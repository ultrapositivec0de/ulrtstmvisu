...
function App() {
    ...
    if (!isLivePreviewEnabled) {
      if (preview && !preview.querySelector('svg.opacity-40')) {
        preview.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-slate-500 text-sm gap-2 py-12 text-center select-none">
          ...
          <p>${lang === 'uk' ? "–ü—Ä–µ–≤'—é –≤–∏–º–∫–Ω–µ–Ω–æ –¥–ª—è –µ–∫–æ–Ω–æ–º—ñ—ó —Ä–µ—Å—É—Ä—Å—ñ–≤.<br>–ù–∞—Ç–∏—Å–Ω—ñ—Ç—å –∫–Ω–æ–ø–∫—É –∑ –æ–∫–æ–º, —â–æ–± —É–≤—ñ–º–∫–Ω—É—Ç–∏ –∑–Ω–æ–≤—É." : "Live Preview disabled to save resources.<br>Click the eye icon to enable again."}</p>
        </div>`;
      }
      return;
    }
  ...
  const getExifTableFromBlob = async (file: File | Blob): Promise<string> => {
    ...
    try {
      ...
      let table = '\n| Param | Camera Info |\n| --- | --- |\n';
      if (make || model) table += `| üì∏ | ${make} ${model} |\n`;
      if (fNumber) table += `| üîò | ${fNumber} |\n`;
      if (shutter) table += `| ‚è≤Ô∏è | ${shutter} |\n`;
      if (iso) table += `| üéûÔ∏è | ${iso} |\n`;
      if (focal) table += `| üîç | ${focal} |\n`;
  ...
  const createPermlinkUA = (title: string): string => {
    ...
    text = text.replace(/–∑–≥/g, 'zgh'); // –ü—Ä–∞–≤–∏–ª–æ "–∑–≥"
    const specialStart: Record<string, string> = { '—î': 'ye', '—ó': 'yi', '–π': 'y', '—é': 'yu', '—è': 'ya' };
    const specialMid: Record<string, string> = { '—î': 'ye', '—ó': 'yi', '–π': 'y', '—é': 'yu', '—è': 'ya' };
    const standardMap: Record<string, string> = {
        '–∞': 'a', '–±': 'b', '–≤': 'v', '–≥': 'h', '“ë': 'g', '–¥': 'd', '–µ': 'e', '–∂': 'zh',
        '–∑': 'z', '–∏': 'y', '—ñ': 'i', '–∫': 'k', '–ª': 'l', '–º': 'm', '–Ω': 'n', '–æ': 'o',
        '–ø': 'p', '—Ä': 'r', '—Å': 's', '—Ç': 't', '—É': 'u', '—Ñ': 'f', '—Ö': 'kh', '—Ü': 'ts',
        '—á': 'ch', '—à': 'sh', '—â': 'shch', '—å': '', '‚Äô': '', "'": '', ' º': ''
    };
  ...
  const sanitizeFilename = (name: string): string => {
    const ukrToLatin: Record<string, string> = {
      ...
      '—å': '', '—é': 'yu', '—è': 'ya', '–ê': 'A', '–ë': 'B', '–í': 'V', '–ì': 'H', '“ê': 'G', '–î': 'D', '–ï': 'E',
      '–Ñ': 'Ye', '–ñ': 'Zh', '–ó': 'Z', '–ò': 'Y', '–Ü': 'I', '–á': 'Yi', '–ô': 'Y', '–ö': 'K', '–õ': 'L', '–ú': 'M',
      '–ù': 'N', '–û': 'O', '–ü': 'P', '–†': 'R', '–°': 'S', '–¢': 'T', '–£': 'U', '–§': 'F', '–•': 'Kh', '–¶': 'Ts',
  ...
  const [bookStructure] = useState<{ id: string; title: string; chapters: { id: string; title: string; note?: string }[] }[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      {
        id: '1',
        title: '–†–æ–∑–¥—ñ–ª 1: –í—Å—Ç—É–ø —ñ –ö–æ–Ω—Ü–µ–ø—Ü—ñ—è',
        chapters: [
          { id: '1-1', title: '–ì–ª–∞–≤–∞ 1.1: –ü–µ—Ä—à—ñ –ö—Ä–æ–∫–∏', note: '–û–≥–ª—è–¥ —Å—Ç—Ä—É–∫—Ç—É—Ä–∏ —Ç–∞ –≥–æ–ª–æ–≤–Ω–∏–π –ø–æ—Å–∏–ª' },
          { id: '1-2', title: '–ì–ª–∞–≤–∞ 1.2: –û—Å–Ω–æ–≤–Ω–∞ –ú–µ—Ç–∞', note: '–û–ø–∏—Å —ñ –∞–Ω–∞–ª—ñ–∑ –≤–∞—à–∏—Ö —ñ–¥–µ–π' }
        ]
      },
      {
        id: '2',
        title: '–†–æ–∑–¥—ñ–ª 2: –ï–∫—Å–ø–µ—Ä–∏–º–µ–Ω—Ç–∏ —Ç–∞ –ü—Ä–∞–∫—Ç–∏–∫–∞',
        chapters: [
          { id: '2-1', title: '–ì–ª–∞–≤–∞ 2.1: –î–µ—Ç–∞–ª—å–Ω–∏–π –æ–ø–∏—Å', note: '–ö–æ—Ä–∏—Å–Ω–µ —Ñ–æ—Ä–º–∞—Ç—É–≤–∞–Ω–Ω—è —Ç–∞ –ø–æ—Å–∏–ª–∞–Ω–Ω—è' }
        ]
      }
    ];
  });
        ...
        if (m) {
          ...
          if (firstEl && blockTags.includes(firstEl.tagName) && !firstEl.classList.contains('top-spacer')) {
             if (!tempDiv.querySelector('.top-spacer')) {
                ...
                pBefore.className = 'table-spacer top-spacer';
                pBefore.setAttribute('data-empty', 'true');
                pBefore.setAttribute('data-placeholder', lang === 'uk' ? '‚Üµ –ù–æ–≤–∏–π –ø–∞—Ä–∞–≥—Ä–∞—Ñ...' : lang === 'es' ? '‚Üµ Nuevo p√°rrafo...' : lang === 'ko' ? '‚Üµ ÏÉà Î¨∏Îã®...' : '‚Üµ New paragraph...');
                pBefore.innerHTML = '<br>';
                tempDiv.insertBefore(pBefore, firstEl);
             }
          }
          ...
          if (lastEl && blockTags.includes(lastEl.tagName) && !lastEl.classList.contains('bottom-spacer')) {
             if (!tempDiv.querySelector('.bottom-spacer')) {
                ...
                pAfter.className = 'table-spacer bottom-spacer';
                pAfter.setAttribute('data-empty', 'true');
                pAfter.setAttribute('data-placeholder', lang === 'uk' ? '‚Üµ –ù–æ–≤–∏–π –ø–∞—Ä–∞–≥—Ä–∞—Ñ...' : lang === 'es' ? '‚Üµ Nuevo p√°rrafo...' : lang === 'ko' ? '‚Üµ ÏÉà Î¨∏Îã®...' : '‚Üµ New paragraph...');
                pAfter.innerHTML = '<br>';
                tempDiv.appendChild(pAfter);
             }
          }
  ...
  const handleEditorKeyDown = useCallback((e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Enter') {
      ...
      if (activeRange) {
        ...
        // Case B: Caret is at the end of formatted content (e.g. *–∫—É—Ä—Å–æ—Ä —Ç–µ–∫—Å—Ç|*) -> close tag tightly and continue onto new line
        ...
        // Case C: Caret is in the middle of formatted text (e.g. *–ø–µ—Ä—à–∞ —á–∞—Å—Ç–∏–Ω–∞| –¥—Ä—É–≥–∞ —á–∞—Å—Ç–∏–Ω–∞*) -> split tags cleanly and tightly
  ...
  const handleMarkdownFormat = useCallback((tag: string, closeTag: string = tag) => {
    ...
    if (matchingRange) {
      ...
      // 2. If caret is at or after the end of formatted content (e.g. **—ñ–∞–≤—ñ| ** or ** —ñ–∞–≤—ñ **|), exit/finish format and jump out
  ...
  useEffect(() => {
    ...
    // –î–æ–¥–∞—î–º–æ –Ω–µ–≤–µ–ª–∏–∫—É –∑–∞—Ç—Ä–∏–º–∫—É –ø–µ—Ä–µ–¥ –∞–∫—Ç–∏–≤–∞—Ü—ñ—î—é –∑–±–µ—Ä–µ–∂–µ–Ω–Ω—è
    setTimeout(() => {
  ...
  const insertImage = (url: string, name: string, position: 'left' | 'right' | 'center' | 'plain') => {
    ...
    let attribution = '';
    if (gridWithCaptions) {
      attribution = `<div align="${singleCaptionAlign}"><sup> ‚úçÔ∏è </sup></div>`;
    }
    ...
    if (gridLayout === 'col-img-text' || gridLayout === 'col-text-img') {
      ...
      if (imageInsertFormat === 'markdown') {
        const header = isImgFirst 
          ? `| ${t('image') || '–ó–æ–±—Ä–∞–∂–µ–Ω–Ω—è'} | ${t('description') || '–û–ø–∏—Å'} |\n|---|---|\n`
          : `| ${t('description') || '–û–ø–∏—Å'} | ${t('image') || '–ó–æ–±—Ä–∞–∂–µ–Ω–Ω—è'} |\n|---|---|\n`;
        let rows = '';
        selected.forEach((item, index) => {
          const imgCell = generateCell(item, index, galleryMode === 'local', false);
          const descCell = `${t('typeHere') || ' ‚úçÔ∏è '}`;
  ...
  const handleSplitPost = () => {
    ...
    const newDrafts: Draft[] = parts.map((partContent, index) => ({
      ...
      title: `${originalTitle} ‚Ññ${index + 1}`,
      body: `# ${originalTitle} ‚Ññ${index + 1}\n\n${partContent}`,
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setPubLog({ msg: `‚ùå ${t('error')}: ${err.message}`, type: 'error' });
    }
  };
  ...
  const handleClearCache = async () => {
    const confirmed = await confirmDialog(
      lang === 'uk' 
        ? "–û—á–∏—Å—Ç–∏—Ç–∏ –∫–µ—à –ø–µ—Ä–µ–≥–ª—è–Ω—É—Ç–∏—Ö –¥–æ–ø–∏—Å—ñ–≤, —Ç–∏–º—á–∞—Å–æ–≤–∏—Ö —Å–ø–∏—Å–∫—ñ–≤ —Ç–∞ –∑–∞–≤–∞–Ω—Ç–∞–∂–µ–Ω–∏—Ö –∑–æ–±—Ä–∞–∂–µ–Ω—å? –í–∞—à—ñ —á–µ—Ä–Ω–µ—Ç–∫–∏, —à–∞–±–ª–æ–Ω–∏ —Ç–∞ –∑–±–µ—Ä–µ–∂–µ–Ω—ñ –∫–ª—é—á—ñ –ù–ï –±—É–¥—É—Ç—å –≤–∏–¥–∞–ª–µ–Ω—ñ." 
    ...
    try {
      ...
      if (cleared) {
        notify(lang === 'uk' ? "–ö–µ—à –ø–µ—Ä–µ–≥–ª—è–¥—É —Ç–∞ –∑–æ–±—Ä–∞–∂–µ–Ω—å —É—Å–ø—ñ—à–Ω–æ –æ—á–∏—â–µ–Ω–æ!" : "Cache cleared successfully!", "success");
      }
    } catch (err: any) {
      console.error(err);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // –°–ø–µ—Ü—ñ–∞–ª—å–Ω–∞ —Ñ—É–Ω–∫—Ü—ñ—è –∑–∞–≤–∞–Ω—Ç–∞–∂–µ–Ω–Ω—è –ó –ü–†–û–ì–†–ï–°–û–ú –¥–ª—è VAULT (XHR)
    ...
    // –°–¢–†–û–ì–ê –ü–û–°–õ–Ü–î–û–í–ù–ê –ß–ï–†–ì–ê
      ...
      try {
        ...
        if (uploadAuthType === 'VAULT') {
          ...
          while (attempt < 3 && !uploaded) {
            ...
            try {
              setPubLog({ 
                msg: `[${i + 1}/${files.length}] ` + t('signingImage').replace('{name}', file.name) + (attempt > 1 ? ` (—Å–ø—Ä–æ–±–∞ ${attempt})` : ''), 
                type: 'loading' 
              });
              signature = await SecurityService.signBuffer(dataToSign, activeUser);
        ...
        console.error(err);
        setPubLog({ msg: `‚ùå ${t('fileError')} ${i + 1}: ${file.name} - ${err.message}`, type: 'error' });
  ...
  const TOOLS_MAP: Record<string, { label: string | React.ReactNode, action: (e?: React.MouseEvent) => void }> = {
    ...
    'HR': { label: '‚Äî', action: () => insertAtCursor('\n\n---\n\n') },
    ...
    'Caption': { label: t('captionShort'), action: async () => {
      ...
      insertAtCursor(`<center>\n\n| <center>![image](${url})</center> |\n|:---:|\n| <center><sub>${cap || ' ‚úçÔ∏è '}</sub></center> |\n\n</center>\n`);
    }},
    'Left': { label: '‚¨Ö', action: () => fmt('<div class="text-left">\n', '\n</div>') },
    'Center': { label: '–¶–µ–Ω—Ç—Ä', action: () => fmt('<center>\n', '\n</center>') },
    'Right': { label: '‚û°', action: () => fmt('<div class="text-right">\n', '\n</div>') },
    'Justify': { label: '–í–∏—Ä—ñ–≤', action: () => fmt('<div class="text-justify">\n', '\n</div>') },
    'Grid': { label: '–°—ñ—Ç–∫–∞', action: () => insertAtCursor(`<div class="pull-left">\n${t('leftContent')}\n</div>\n<div class="pull-right">\n${t('rightContent')}\n</div>\n<div class="clearfix"></div>\n`) },
              ...
              {showNotificationList && (
                ...
                >
                  <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
                    <div className="flex flex-col">
                      <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">–°–ø–æ–≤—ñ—â–µ–Ω–Ω—è</h3>
                      <span className="text-[9px] text-slate-500 font-bold">{visibleNotifications.filter(n => !n.isRead).length} –Ω–æ–≤–∏—Ö</span>
                    </div>
                    <div className="flex items-center gap-2">
                       ...
                       <button onClick={() => markAllAsRead()} className="p-1 text-slate-500 hover:text-white" title="–û—á–∏—Å—Ç–∏—Ç–∏"><Trash2 size={16} /></button>
                  ...
                  <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {visibleNotifications.length === 0 ? (
                      <div className="p-8 text-center text-slate-500 text-[10px] uppercase font-black">–ü–æ—Ä–æ–∂–Ω—å–æ</div>
                    ) : (
                      <div className="divide-y divide-slate-800/30">
                        {visibleNotifications.map(n => (
                          <div key={n.id} className={cn("p-3 hover:bg-slate-800/30 transition-colors", !n.isRead && "bg-lime-400/5")}>
                            ...
                            >
                              –ü–ï–†–ï–ì–õ–Ø–ù–£–¢–ò <ArrowRight size={14} />
                            </button>
                          </div>
                        ))}
                      </div>
          ...
          <div className="flex items-center gap-1 bg-slate-800/30 p-1 rounded-xl border border-slate-700/30 shrink-0">
            {!isPwaInstalled && !isTauriEnv() && !isNeutralinoEnv() && (
              <IconButton 
                ...
                title={t('installApp') || "–í—Å—Ç–∞–Ω–æ–≤–∏—Ç–∏ PWA"} 
                        ...
                        >
                          <div className="md:max-h-none overflow-y-auto custom-scrollbar pr-1 pb-1">
                                        ...
                                        >
                                          {(typeof window !== 'undefined' && (window as any).steem_keychain) && (
                                            <option value="" className="bg-slate-900 text-slate-300 py-1">
                                              üõ°Ô∏è {username ? `@${username} (Keychain)` : '@keychain (default)'}
                                            </option>
                                          )}
                                          {vaultAccounts.map(acc => (
                                            <option key={acc} value={acc} className="bg-slate-900 text-slate-200 py-1">
                                              üîë @{acc} (Vault) {!SecurityService.isLocked() ? '‚úì' : 'üîí'}
                                            </option>
                                          ))}
                                        </select>
                                      ) : (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 mt-1.5 shrink-0">
                                  <button
                                    title={t('performanceDesc') || '–í–∏–º–∏–∫–∞—î –¥–µ—è–∫—ñ –∞–Ω—ñ–º–∞—Ü—ñ—ó'}
                                    onClick={() => {
                              ...
                              <div className="flex justify-between sm:justify-center bg-slate-950/50 rounded-lg p-0.5 border border-slate-800/50 hover:border-slate-700/50 transition-colors w-full overflow-x-auto no-scrollbar gap-0.5">
                                  {(['col', 'col-table', 'grid-2', 'row', 'col-img-text', 'col-text-img'] as const).map(l => (
                                    <button
                                      ...
                                      title={l === 'col' ? '–í —Å—Ç–æ–≤–ø—á–∏–∫ (–Ω–µ —Ç–∞–±–ª–∏—Ü—è)' : l === 'col-table' ? '–°—Ç–æ–≤–ø—á–∏–∫ –≤ —Ç–∞–±–ª–∏—Ü—ñ (–∑–≤–µ—Ä—Ö—É –≤–Ω–∏–∑)' : l === 'grid-2' ? '–ü–ª–∏—Ç–∫–∞ (2x2)' : l === 'row' ? '–í –æ–¥–∏–Ω —Ä—è–¥–æ–∫ (—Ç–∞–±–ª–∏—Ü—è)' : l === 'col-img-text' ? '–¢–µ–∫—Å—Ç –ø—Ä–∞–≤–æ—Ä—É—á' : '–¢–µ–∫—Å—Ç –ª—ñ–≤–æ—Ä—É—á'}
                                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      notify(
                        next 
                           ? (lang === 'uk' ? "–£–≤—ñ–º–∫–Ω–µ–Ω–æ –æ–ø—Ç–∏–º—ñ–∑–æ–≤–∞–Ω—É —Ñ–æ–Ω–æ–≤—É —Å–∏–Ω—Ö—Ä–æ–Ω—ñ–∑–∞—Ü—ñ—é (–±–µ–∑ –∑–∞—Ç—Ä–∏–º–æ–∫)" : "Optimized background sync enabled (lag-free)") 
                           : (lang === 'uk' ? "–£–≤—ñ–º–∫–Ω–µ–Ω–æ –±–µ–∑–ø–µ—Ä–µ—Ä–≤–Ω—É —Å–∏–Ω—Ö—Ä–æ–Ω—ñ–∑–∞—Ü—ñ—é –≤ —Ä–µ–∞–ª—å–Ω–æ–º—É —á–∞—Å—ñ" : "Continuous real-time sync enabled"), 
                        "success"
                      );
                    }}
                    className={cn(
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      notify(
                        next
                          ? (lang === 'uk' ? "–ë'—é—Ç—ñ—Ñ—ñ–∫–∞—Ü—ñ—é —É–≤—ñ–º–∫–Ω–µ–Ω–æ (–ø–æ–∫—Ä–∞—â–µ–Ω–µ –æ—Ñ–æ—Ä–º–ª–µ–Ω–Ω—è)" : "Beautification enabled (enhanced styling)")
                          : (lang === 'uk' ? "–ë'—é—Ç—ñ—Ñ—ñ–∫–∞—Ü—ñ—é –≤–∏–º–∫–Ω–µ–Ω–æ" : "Beautification disabled"),
                        "success"
                      );
                    }}
                    className={cn(
                  ...
                  {visualStyle === 'neon' && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        notify(
                          next 
                            ? (lang === 'uk' ? "–ö–æ–ª—å–æ—Ä–æ–≤–∏–π —Ç–µ–∫—Å—Ç —É–≤—ñ–º–∫–Ω–µ–Ω–æ" : "Colored text enabled") 
                            : (lang === 'uk' ? "–ö–æ–ª—å–æ—Ä–æ–≤–∏–π —Ç–µ–∫—Å—Ç –≤–∏–º–∫–Ω–µ–Ω–æ" : "Colored text disabled"), 
                          "success"
                        );
                      }}
                      className={cn(
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider block">
                                    {lang === 'uk' ? "–ë'—é—Ç—ñ—Ñ—ñ–∫–∞—Ü—ñ—è" : "Beautification"}
                                  </span>
                    ...
                    style={(() => {
                      ...
                      if (widgetPos === 'floating' && window.innerWidth >= 1024 && floatingPos && editorPaneRef.current) {
                        ...
                        if (isKeyboardOpen) {
                          ...
                          const mobileNavEl = typeof document !== 'undefined' ? document.querySelector('nav.lg\\:hidden') as HTMLElement : null;
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between bg-white/5 p-3 rounded-2xl border border-white/5">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-200 uppercase tracking-tight">{t('widgetNoBorder') || '–ë–µ–∑ —Ä–∞–º–æ–∫'}</span>
                                  </div>
                                  <button 
                                    onClick={() => {
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="grid grid-cols-3 gap-1.5">
                                    {[
                                      { id: 'bottom', label: lang === 'uk' ? '–í–ù–ò–ó–£' : 'BOTTOM' },
                                      { id: 'floating', label: lang === 'uk' ? '–ü–õ–ê–í–ê–Æ–ß–ò–ô' : 'FLOAT' },
                                      { id: 'hidden', label: lang === 'uk' ? '–í–ò–ú–ö–ù' : 'OFF' }
                                    ].map(pos => (
                                      <button
                                        key={pos.id}
                                        type="button"
              ...
              <div className="absolute top-4 right-4 z-10 flex gap-2">
                <div className="flex p-1 bg-slate-800 rounded-xl border border-slate-700 gap-1 shrink-0">
                  <button
                    ...
                    title={lang === 'uk' ? "–£–≤—ñ–º–∫–Ω—É—Ç–∏/–≤–∏–º–∫–Ω—É—Ç–∏ –ø—Ä–µ–≤'—é –ø–µ—Ä–µ–≥–ª—è–¥—É" : "Enable/Disable Live Preview"}
                  >
                ...
                <div className="space-y-4 pt-4 border-t border-slate-800">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input 
                          ...
                          placeholder={pexelsApiKey ? "‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢" : t('pexelsKey')}
                          value={tempPexelsKey}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input 
                          type="password" 
                          className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm focus:ring-1 focus:ring-cyan-500 outline-none transition-all"
                          placeholder={pixabayApiKey ? "‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢" : t('pixabayKey')}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input 
                          type="password" 
                          className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-sm focus:ring-1 focus:ring-cyan-500 outline-none transition-all"
                          placeholder={unsplashAccessKey ? "‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢‚Ä¢" : t('unsplashAccessKey')}
                    ...
                    <div className="pt-2">
                       <button 
                         onClick={async () => {
                           if (await confirmDialog(t('confirmClearApiKeys') || "–û—á–∏—Å—Ç–∏—Ç–∏ –≤—Å—ñ API –∫–ª—é—á—ñ?")) {
                             ...
                             notify(t('keysCleared') || "API –∫–ª—é—á—ñ –æ—á–∏—â–µ–Ω–æ!");
                           }
                         }}
                         className="text-[10px] text-red-500 hover:text-red-400 underline"
                       >
                         {t('clearApiKeys') || "–û—á–∏—Å—Ç–∏—Ç–∏ API –∫–ª—é—á—ñ"}
                       </button>
                    </div>
                  </div>
                </div>
                      ...
                      {!(window as any).steem_keychain && (
                        <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-[10px] text-amber-400 flex items-start gap-2.5">
                          ...
                          <p className="leading-relaxed">
                            {lang === 'uk' 
                              ? 'Steem Keychain –Ω–µ –∑–Ω–∞–π–¥–µ–Ω–æ. –†–µ–∫–æ–º–µ–Ω–¥—É—î–º–æ –∑–Ω–∞–π—Ç–∏ –π–æ–≥–æ —É –º–∞–≥–∞–∑–∏–Ω–∞—Ö —Ä–æ–∑—à–∏—Ä–µ–Ω—å –¥–ª—è –±—Ä–∞—É–∑–µ—Ä—ñ–≤ –ü–ö, –∞ –¥–ª—è –º–æ–±—ñ–ª—å–Ω–∏—Ö ‚Äî —É –≤—ñ–¥–ø–æ–≤—ñ–¥–Ω–∏—Ö –º–∞—Ä–∫–µ—Ç–∞—Ö –∑–∞—Å—Ç–æ—Å—É–Ω–∫—ñ–≤.' 
                            ...
                            {mentions.length > 0 && (
                              <div className="p-3 bg-slate-800/30 rounded-xl border border-slate-800">
                                <div className="flex justify-between items-center mb-2">
                                  ...
                                  <button onClick={() => setActiveModal('mentions')} className="text-[8px] text-cyan-400 hover:underline px-1 uppercase font-bold">–†–µ–¥–∞–≥—É–≤–∞—Ç–∏ —Å–ø–∏—Å–æ–∫</button>
                                </div>
                ...
                <div className="px-4 py-2 bg-slate-800/10 border border-slate-800 rounded-xl flex items-center justify-between opacity-80 hover:opacity-100 transition-opacity">
                   <div className="flex items-center gap-2">
                      <AtSign size={16} className={cn(
                        "transition-colors",
                        (contentForPublish.includes('‚úçÔ∏è') || contentForPublish.includes('center') || contentForPublish.toLowerCase().includes('signature')) ? "text-green-500" : "text-slate-600"
                      )} />
                      <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">{t('signaturePolicy')}</span>
                   </div>
                   <div className="flex items-center gap-2">
                      { (contentForPublish.includes('‚úçÔ∏è') || contentForPublish.includes('center') || contentForPublish.toLowerCase().includes('signature')) ? (
                ...
                <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500